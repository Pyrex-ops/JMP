...
credentialsManager::credentialsManager(const char* namespaceName){
    if(namespaceName[0]!='\0'){ //Controlliamo che il vettore di caratteri fornito non sia vuoto
...
bool credentialsManager::newRecord(String SSID, String password){
    //Non supportate reti senza password (lunghezza minima 8 caratteri, richiesto da WPA)
    if(SSID.length()<2 || password.length()<8 || SSID.length()>32 || password.length()>63){
        return false;
    }
    ...
    if(preferences.putString("ssid",charSSID)>0 && preferences.putString("password",charPASSWORD)>0){
        delete charPASSWORD;
        delete charSSID;
