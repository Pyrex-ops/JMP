...
MotorinoGravity::MotorinoGravity(
	uint8_t motorinoPin, uint8_t minimumIntensity, uint8_t ledcChannel,
	uint32_t ledcFrequency, uint8_t ledcResolution, uint8_t taskPriority,
	uint8_t taskCore, uint16_t tempoFraVibrazioni)
	: PIN(motorinoPin), MINIMUM_INTENSITY(minimumIntensity),
	  LEDC_CHANNEL(ledcChannel), LEDC_FREQ(ledcFrequency),
	  LEDC_RESOLUTION(ledcResolution), TASK_CORE(taskCore) {
...
void MotorinoGravity::vibraIntermittenteTask(void* informazioniVibrazioneIntermittenteVoid) {
	InformazioniVibrazioneIntermittente* infoVibrazione =
		(InformazioniVibrazioneIntermittente*)(informazioniVibrazioneIntermittenteVoid);
	pinMode(infoVibrazione->infoAcceso.pin, OUTPUT);
	ledcSetup(infoVibrazione->infoAcceso.ledcChannel, infoVibrazione->infoAcceso.ledcFrequency,
			  infoVibrazione->infoAcceso.ledcResolution);
	...
	for (int i = 0; i < infoVibrazione->numeroPeriodi; i++) {
		ledcWrite(infoVibrazione->infoAcceso.ledcChannel,
				  infoVibrazione->infoAcceso.intensity);
	...
	if (!MotorinoGravity::infoQueue.empty()) {
		...
		xTaskCreatePinnedToCore(
			vibraIntermittenteTask, "vibrazioneIntermittente", 4096, taskArg,
			MotorinoGravity::TASK_PRIORITY, &MotorinoGravity::taskVibrazione, 0);
	} else {
...
void MotorinoGravity::vibraIntermittente(
	uint16_t millisecondiSpento, uint16_t millisecondiAceso,
	uint16_t numeroPeriodi, uint16_t intensity) {
	InformazioniVibrazioneIntermittente* infoVibrazione = new InformazioniVibrazioneIntermittente;
	if (intensity < MINIMUM_INTENSITY) intensity = MINIMUM_INTENSITY;
	...
	if (taskVibrazione != NULL) {
		...
		xTaskCreatePinnedToCore(
			vibraIntermittenteTask, "vibrazioneIntermittente", 4096,
			(void*)infoVibrazione, TASK_PRIORITY, &taskVibrazione, 0);
	}
}
...
void MotorinoGravity::vibraIntermittente(
	uint16_t millisecondiSpento, uint16_t millisecondiAceso, uint16_t numeroPeriodi) {
	vibraIntermittente(millisecondiSpento, millisecondiAceso, numeroPeriodi,
					   (1 << LEDC_RESOLUTION) - 1);
}
